	...
	return (
		...
		>
			...
			<div className="flex flex-col justify-between lg:col-span-2">
				<div className="relative h-full space-y-6 px-4 py-8 md:p-8">
			...
			<div
				className={cn(
					'bg-muted/40 flex h-full w-full items-center justify-center border-t p-6 md:col-span-1 md:border-t-0 md:border-l',
					formSectionClassName,
				)}
			>
				{children}
...
function ContactInfo({
	...
	return (
		<div className={cn('flex items-start gap-3', className)} {...props}>
			<div className="bg-muted/40 rounded-lg p-3 flex-shrink-0">
				<Icon className="h-5 w-5" />
			</div>
			<div className="min-w-0 flex-1">
				<p className="font-medium mb-1">{label}</p>
				<p className={cn("text-muted-foreground text-xs leading-relaxed", isEmail ? 'whitespace-nowrap' : 'break-words')}>{value}</p>
			</div>
		</div>
	);
}
